...
class JSNUniformHelper
{
		...
		{
				$type = str_replace(array("password", "email", "single-line-text", "file-upload", "currency", "phone", "website", "dropdown", "choices", "date", "country", "number"), "varchar(255)", $type);
		...
		public static function generateHTMLPages($formId, $formName, $formType = "")
		{
				...
				$document->addScriptDeclaration('var jsn_form_language =' . json_encode(self::getTranslated($arrayTranslated)));
				$document->addScriptDeclaration('var jsn_form_settings =' . json_encode($settings));
				$document->addScriptDeclaration('var jsn_form_pathRoot ="' . JURI::root() . '"');
				...
				if ($items)
				{
						...
						if (!$formType)
						{
								$document->addStyleSheet(JRoute::_('index.php?option=com_uniform&task=form.save&view=form&task=generateStylePages&form_id=' . $items->form_id));
								$html .= "<div class=\"jsn-uniform jsn-master\" id='jsn_form_" . $items->form_id . "'><div class=\"jsn-bootstrap\">";
								$html .= "<form name='form_{$formName}' id='form_{$formName}' action=\"" . JRoute::_('index.php?option=com_uniform&view=form&task=form.save&form_id=' . $items->form_id) . "\" method=\"post\" class=\"form-validate {$classForm} \" enctype=\"multipart/form-data\" >";
