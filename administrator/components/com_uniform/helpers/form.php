...
class JSNFormGenerateHelper
{
		...
		public static function fieldDate($data, $dataSumbission)
		{
				...
				$html = "<div class=\"control-group {$identify} {$hideField}\">
					<label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label>
		...
		public static function fieldCurrency($data, $dataSumbission)
		{
				...
				if (isset($settings->options->format))
				{
						...
						if ($settings->options->format != "Yen" && $settings->options->format != "Rupee")
						{
								$showHelpBlockSents = "";
								if (!empty($settings->options->showCurrencyTitle) && $settings->options->showCurrencyTitle == "Yes")
								{
		...
		public static function fieldPhone($data, $dataSumbission)
		{
				...
				$html = "<div class=\"control-group {$identify} {$hideField}\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\">{$inputContent}</div></div>";
				return $html;
		}
		...
		public static function fieldName($data, $dataSumbission)
		{
				...
				$requiredBlank = !empty($settings->options->required) ? 'group-blank-required' : '';
				$instruction = !empty($settings->options->instruction) ? " <i original-title=\"" . htmlentities(JText::_($settings->options->instruction), ENT_QUOTES, "UTF-8") . "\" class=\"icon-question-sign\"></i>" : '';
				$html = "<div class=\"control-group {$identify} {$hideField}\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div id=\"{$data->field_id}\" class=\"controls {$requiredBlank}\">";
				$valueFirstName = '';
				$valueLastName = '';
				$valueMiddle = '';
		...
		public static function fieldFileUpload($data, $dataSumbission)
		{
				...
				$required = !empty($settings->options->required) ? '<span class="required">*</span>' : '';
				$instruction = !empty($settings->options->instruction) ? " <i original-title=\"" . htmlentities(JText::_($settings->options->instruction), ENT_QUOTES, "UTF-8") . "\" class=\"icon-question-sign\"></i>" : '';
				$html = "<div class=\"control-group {$identify} {$hideField}\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><input id=\"{$data->field_id}\" class=\"input-file\" name=\"{$data->field_id}\" type=\"file\" /></div></div>";
				return $html;
		}
		...
		public static function fieldEmail($data, $dataSumbission)
		{
				...
				$html = "<div class=\"control-group {$identify} {$hideField}\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls\">";
		...
		public static function fieldDropdown($data, $dataSumbission)
		{
				...
				$html = "<div class=\"control-group {$identify} {$hideField}\" data-settings=\"" . htmlentities($dataSettings, ENT_QUOTES, "UTF-8") . "\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><select id=\"{$data->field_id}\" class=\"dropdown {$sizeInput} {$randomDropdown}\" name=\"{$data->field_id}\">";
		...
		public static function fieldCountry($data, $dataSumbission)
		{
				...
				$defaultValue = !empty($dataSumbission[$data->field_id]) ? $dataSumbission[$data->field_id] : "";
				$sizeInput = !empty($settings->options->size) ? $settings->options->size : '';
				$html = "<div class=\"control-group {$identify} {$hideField} \"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><select id=\"{$data->field_id}\" class=\"{$sizeInput}\" name=\"{$data->field_id}\">";
				if (isset($settings->options->items) && is_array($settings->options->items))
				{
						foreach ($settings->options->items as $option)
						{
		...
		public static function fieldAddress($data, $dataSumbission)
		{
				...
				$html = "<div class=\"control-group jsn-group-field {$identify} {$hideField} \"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div id=\"{$data->field_id}\" class=\"controls {$requiredBlank}\">";
		...
		public static function fieldPassword($data, $dataSumbission)
		{
				...
				$styleClassLimit = "";
				$identify = !empty($settings->identify) ? $settings->identify : "";
				if (isset($settings->options->limitation) && $settings->options->limitation == 1 && $settings->options->limitMax > 0)
				{
						$josnLimit = json_encode(array('limitMin' => $settings->options->limitMin, 'limitMax' => $settings->options->limitMax));
				...
				$sizeInput = !empty($settings->options->size) ? $settings->options->size : '';
				$placeholder = !empty($settings->options->value) ? JText::_($settings->options->value) : "";
				$placeholderConfirm = !empty($settings->options->valueConfirmation) ? JText::_($settings->options->valueConfirmation) : "";
				$confirmHtml = "";
				if (!empty($settings->options->confirmation))
				{
						$confirmHtml = "</br><input {$limitValue} class=\"{$sizeInput}\" name=\"password[{$data->field_id}][]\" type=\"password\" value=\"\" placeholder=\"" . htmlentities($placeholderConfirm, ENT_QUOTES, "UTF-8") . "\" />";
				}
				$html = "<div class=\"control-group jsn-group-field {$identify} {$hideField}\">
		<label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label>
			<div class=\"controls {$requiredBlank} {$styleClassLimit}\" id=\"{$data->field_id}\" >
				<input {$limitValue} class=\"{$sizeInput}\" name=\"password[{$data->field_id}][]\" type=\"password\" value=\"\" placeholder=\"" . htmlentities($placeholder, ENT_QUOTES, "UTF-8") . "\" />
				{$confirmHtml}
			</div>
		</div>";
				return $html;
