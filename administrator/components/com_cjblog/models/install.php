...
class CjBlogModelInstall extends JModelLegacy {
	...
	function createMenu() {
		...
		$menu = array(
				'name'=>'CjBlog', 'alias'=>'blog', 'link'=>'index.php?option=com_cjblog&view=home', 'access'=>0, 'params'=>array());
		$submenu = array(
	...
	function createMenuJ25($menu, $submenu) {
		...
		if ($this->_db->getErrorNum ()){
			...
			JLog::add('CreateMenuJ25: '.$this->_db->getErrorMsg ().'| Query: '.$query, JLog::ERROR, CJBLOG);
		...
		if (! $table->store ()) {
			JLog::add('CreateMenuJ25: '.$table->getError(), JLog::ERROR, CJBLOG);
			throw new Exception ( $table->getError () );
		}

		$table = JTable::getInstance ( 'Menu' );
		$table->load(array('menutype'=>'cjblogmenu', 'link'=>$menu ['link']));
		...
		if (! $table->setLocation ( 1, 'last-child' ) || ! $table->bind ( $data ) || ! $table->check () || ! $table->store ()) {
			...
			JLog::add('CreateMenuJ25: '.$table->getError().'| Alias: '.$table->alias.'| Table: '.print_r($table, TRUE), JLog::ERROR, CJBLOG);
			$table->alias = 'cjblog';
		...
		foreach ( $submenu as $menuitem ) {
			...
			if (! $table->setLocation ( $parent->id, 'last-child' ) || ! $table->bind ( $data ) || ! $table->check () || ! $table->store ()) {
				JLog::add('CreateMenuJ25: '.$table->getError().'| Table: '.print_r($table, true), JLog::ERROR, CJBLOG);
				throw new Exception ( $table->getError () );
			}
		...
		if (! $parent->check () || ! $parent->store ()) {
			...
			JLog::add('CreateMenuJ25: '.$parent->getError().'| Table: '.print_r($parent, true), JLog::ERROR, CJBLOG);
			throw new Exception ( $table->getError () );
		}
