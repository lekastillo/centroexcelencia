...
class CjBlogModelBadges extends JModelLegacy {
	...
	function import_rules(){
		...
		if(JFile::upload($file['tmp_name'], CJBLOG_PLUGINS_BASE_DIR.'badges'.DS.$filename)){
	...
	function get_rule_type($id, $asset_name, $name){
		...
		$path = JPATH_ROOT.DS.'media'.DS.'cjblog'.DS.'plugins'.DS.'badges'.DS.$type->asset_name.'.xml';
	...
	function save_rule(){
		...
		$path = JPATH_ROOT.DS.'media'.DS.'cjblog'.DS.'plugins'.DS.'badges'.DS.$asset_name.'.xml';
