...
class CjBlogModelPoints extends JModelLegacy {
	...
	function import_rules(){
		...
		$querysub = '
				insert into
					'.T_CJBLOG_POINT_RULES.'(name, title, description, asset_name, auto_approve, access, points, published)
				values ';
		
		foreach ($extensions->extension_rule as $extension_rule){

			$query = $querysub . '(
					'.$this->_db->quote($extension_rule->rule_name).',
					'.$this->_db->quote($extension_rule->rule_title).',
					'.$this->_db->quote($extension_rule->rule_description).',
					'.$this->_db->quote($extension_rule->asset_name).',
					'.intval($extension_rule->auto_approve).',
					'.intval($extension_rule->access_level).',
					0, 0)';
			...
			if(!$this->_db->query()){
				...
				$app->enqueueMessage('Rule '.$extension_rule->rule_name.' not installed as it already exists.');
			}
		}
	...
	public function get_rule($id){
		...
		$query = '
				select 
					id, title, name, description, asset_name, points, auto_approve, published, catid, access
				from
	...
	public function save_rule(){
		...
		$query = '
			update
				'.T_CJBLOG_POINT_RULES.'
			set
				title = '.$this->_db->quote($rule->title).',
				description = '.$this->_db->quote($rule->description).',
				points = '.$rule->points.',
