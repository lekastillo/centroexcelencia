				...
				foreach ($columsSubmission as $colum)
				{
						if (!in_array($colum->field_id, $fieldClear))
						{
								if ($colum->field_type != 'static-content')
								{
										...
										{
												$value = $this->_db->Quote($this->_fieldPassword($post, $fieldName, $colum->field_title, $fieldSettings, $validationForm));
												$contetnEmail = $this->_fieldPassword($post, $fieldName, $colum->field_title, $fieldSettings, $validationForm);
										}
										$values[] = trim($value);
										...
										if (isset($fieldSettings->options->required) && (int) $fieldSettings->options->required == 1)
										{
												switch($colum->field_type)
												{
														...
														case "phone":
																...
																break;
														case "password":
																//print_r(count($post['password'][$fieldName]));
																if (count($post['password'][$fieldName]) > 1)
																{
																		if (empty($post['password'][$fieldName][0]) || empty($post['password'][$fieldName][1]))
																		{
																				$validationForm['password'][$fieldName] = JText::_('JSN_UNIFORM_CONFIRM_FIELD_CANNOT_EMPTY');
																		}
																		else if (!empty($post['password'][$fieldName][0]) && !empty($post['password'][$fieldName][1]) && $post['password'][$fieldName][0] != $post['password'][$fieldName][1])
																		{
																...
																{
																		if (empty($post['password'][$fieldName][0]))
																		{
																				$validationForm['password'][$fieldName] = JText::_('JSN_UNIFORM_CONFIRM_FIELD_CANNOT_EMPTY');
																		}
																}
