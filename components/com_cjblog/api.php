...
require_once JPATH_ROOT.DS.'components'.DS.CJBLOG.DS.'helpers'.DS.'constants.php';
...
class CjBlogApi {
	...
	public static function load_users(array $identifiers = array(), $force_reload = false){
		...
		if(!empty($notfound)){
			...
			$query = '
				select
					u.id, u.avatar, u.about, u.points, u.num_articles, u.num_badges, u.country, u.profile_views,
	...
	public static function get_badges_by_component_name($asset_name){
	
		$db = JFactory::getDbo();
	
		$query = '
			select
				count(*) as num_times, b.id, m.date_assigned, m.user_id,
				b.name, b.title, b.description, b.icon, b.published, b.css_class
			from
				'.T_CJBLOG_USER_BADGE_MAP.' m
			left join
				'.T_CJBLOG_BADGES.' b on b.id = m.badge_id
			where
				b.asset_name = '.$db->quote($asset_name).'
			group by
				m.badge_id
			order by
				m.date_assigned desc';
	...
	public static function award_points($rule_name, $user_id=0, $points=0, $reference=null, $description=null){
		...
		$query = '
			select 
				id, name, asset_name, description, points, published, auto_approve, access
			from 
	...
	public static function trigger_badge_rule($name, array $params, $userid = 0){
		...
		if(empty(CjBlogApi::$_badge_rules)){
			...
			if(!empty($rules)){
				...
				JLog::add('Trigger Badge Rule - No Errors Loaded. DB Error: '.$db->getErrorMsg(), JLog::ERROR, CJBLOG);
			}
		}
