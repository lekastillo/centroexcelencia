...
class CjBlogViewUsers extends JViewLegacy {
	...
	function display($tpl = null) {
		...
		if(!empty($result->users)){
			...
			$system = $this->params->get('user_avatar', 'cjblog');
			CJFunctions::load_users($system, $userids);
			
			foreach ($result->users as &$user){
				
				$user['avatar'] = CJFunctions::get_user_avatar($system, $user['id'], $this->params->get('user_display_name', 'name'), 64);
				$user['link'] = CJFunctions::get_user_profile_link($system, $user['id'], $user[$this->params->get('user_display_name', 'name')]);
			}
		}
