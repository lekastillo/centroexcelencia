...
<div id="cj-wrapper">
	...
	<div class="profile-container">
		...
		<div class="container-fluid">	
			<div class="row-fluid">
				<div class="span3 vcard">
					<ul class="thumbnails nospace-left nospace-bottom nospace-top">
						<li class="span12 thumbnail avatar-wrapper nomargin-left">
							<a class="span12" onclick="return false;" href="#"><img class="avatar photo" src="<?php echo $this->profile['avatar'];?>" alt="<?php echo $this->profile['name']?>"></a>
				...
				<div class="span9">
					...
					<p class="user-about"><?php echo $this->escape($this->profile['about']);?></p>
					...
					<form id="edit-about-form" action="<?php echo JRoute::_('index.php?option='.CJBLOG.'&view=profile&task=save_about&id='.$this->profile['id'].$profile_itemid);?>" style="display: none;">
						<textarea name="user-about" cols="20" rows="5" class="span9"><?php echo $this->escape($this->profile['about']);?></textarea>
						<p>
							<button class="btn btn-small btn-edit-about" type="button"><i class="icon-remove"></i> <?php echo JText::_('LBL_CANCEL');?></button>
							<button class="btn btn-small btn-success btn-save-about" type="button" data-loading-text="<?php echo JText::_('LBL_WAIT');?>"><i class="icon-ok icon-white"></i> <?php echo JText::_('LBL_SAVE');?></button>
						</p>
					</form>
					<?php endif;?>
					
					<div class="muted"><?php echo JText::sprintf('TXT_REGISTERED_DATE', CJFunctions::get_localized_date($this->profile['registerDate'], 'F j, Y, g:i a'));?></div>
					<div class="muted"><?php echo JText::sprintf('TXT_LAST_VISITED_DATE', CJFunctions::get_localized_date($this->profile['lastvisitDate'], 'F j, Y, g:i a'));?></div>
		...
		<div id="modal-change-avatar" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="model-label" aria-hidden="true" style="display: none;">>
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
